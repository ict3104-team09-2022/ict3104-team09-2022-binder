...
def main():
    ...
    if args.num_classes == 60:
        label_map = os.path.join(os.getcwd(),"step",args.data_root, 'label/ava_action_list_v2.1_for_activitynet_2018.pbtxt')
    ...
    dataset = AVADataset(os.path.join(os.getcwd(),"step",args.data_root), 'test', args.input_type, args.T, args.NUM_CHUNKS[args.max_iter], args.fps, BaseTransform(args.image_size, args.means, args.stds,args.scale_norm), proposal_path=args.proposal_path_val, stride=1, anchor_mode=args.anchor_mode, num_classes=args.num_classes, foreground_only=False)
    ...
    for i in range(args.max_iter):
        ...
        metrics = ava_evaluation(os.path.join(os.getcwd(),"step",args.data_root, 'label/'), output_files[i], gt_file)
