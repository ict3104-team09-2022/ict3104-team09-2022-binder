...
def val_step(model, gpu, dataloader, epoch):
    ...
    current = apm.value().tolist()
    for value in current:
          rows.append([float(val_map)-value, value])
          end = True

    filename = "/Evaluation.csv"
    with open(args.outputTest + filename, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(fields)
        csv_writer.writerows(rows)
        print('Evaluation Done. You can find the result in ' + args.outputTest + filename)
